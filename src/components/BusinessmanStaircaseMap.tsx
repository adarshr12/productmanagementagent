...
export function BusinessmanStaircaseMap({
  ...
  // Accordion: which stair step (mobile & tablet only) has its detail
  // panel open. Single-open, like a standard accordion — opening one
  // closes whichever was open before.
  const [openStepId, setOpenStepId] = useState<string | null>(null);
  // Below desktop the illustration itself renders nothing (see
  // BusinessmanPathAnimation) — the scroll-driven stair map becomes a
  // plain static checklist there instead, with no animation, image, or
  // video of any kind.
  ...
  useGSAP(
    () => {
      if (isControlled || !isDesktop || !containerRef.current) return;
    ...
    { scope: containerRef, dependencies: [steps.length, isControlled, isDesktop] }
  );

  return (
    <div
      ref={containerRef}
      className="relative w-full rounded-3xl border border-line bg-gradient-to-b from-white via-paper to-accent-50/40 p-4 sm:p-8 shadow-[0_20px_50px_-20px_rgba(16,25,43,0.14)] overflow-hidden flex flex-col justify-center h-full min-h-[340px] sm:min-h-[540px]"
    >
      ...
      {/* Header Bar */}
      <div className="mb-3 sm:mb-4 flex items-center justify-between border-b border-line/80 pb-2 sm:pb-3">
      ...
      {isDesktop ? (
        /* Businessman Path Animation Container */
        ...
        /* Mobile & tablet: no illustration, no animation — each stair step
           is instead an accordion row. Collapsed it matches the desktop
           pill (number, label, duration); expanding it reveals the same
           title/description/skill-badge detail the desktop pills only
           imply, with no motion beyond a plain CSS height transition. */
        <div className="flex flex-col gap-2.5">
          {steps.map((stair, idx) => {
            const isGreen = idx < completedCount;
            const isOpen = openStepId === stair.id;
            return (
              <div
                key={stair.id}
              ...
              >
                <button
                  type="button"
                  onClick={() => setOpenStepId(isOpen ? null : stair.id)}
                  aria-expanded={isOpen}
                  className={`flex w-full items-center gap-3 px-3.5 py-2.5 text-left text-sm font-semibold ${
                ...
                >
                  <span
                    className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-extrabold ${
                      isGreen ? "bg-emerald-500 text-white" : "bg-line text-slate"
                    }`}
                  >
                    {isGreen ? "✓" : stair.stepNumber}
                  </span>
                  <span className="min-w-0 flex-1 truncate font-mono text-xs tracking-tight sm:text-[13px]">
                    {stair.stairLabel}
                  </span>
                  <span className="shrink-0 text-[11px] font-medium text-slate-soft">
                    {stair.duration}
                  </span>
                  <ChevronDown
                    className={`h-4 w-4 shrink-0 text-slate-soft transition-transform duration-200 ${
                      isOpen ? "rotate-180" : ""
                    }`}
                  />
                </button>
                {isOpen && (
                  <div className="border-t border-line/70 px-3.5 pb-3.5 pt-3">
                    <h4 className="text-sm font-bold text-ink">{stair.title}</h4>
                    <p className="mt-1 text-xs leading-relaxed text-slate">
                      {stair.description}
                    </p>
                    <span className="pill mt-2 inline-flex border border-line bg-paper text-slate-soft">
                      {stair.skillBadge}
                    </span>
                  </div>
                )}
